...
function makeGrid() {
  	...
  	if(gridHeight >= 96 || gridWidth >= 96) {
		$(".canvas td").css("height", "13px");
		$(".canvas td").css("min-width", "13px");
	} else if(gridHeight >= 72 || gridWidth >= 72) {
...
$("#load").click(function(){
	window.open("load", "그림 불러오기", "width=800, height=460, fullscreen=no, status=no, menubar=no, resizable=no, scrollorbars=no, toolbar=no");
});
...
$("#dummy2").click(function(){
	window.open("save", "그림 저장", "width=400, height=300, directories=no, fullscreen=no, status=no, menubar=no, resizable=no, scrollorbars=no, toolbar=no, location=no");
});
