...
$(function() {
	...
	$('#pw').focusout(function() {
		...
		if (!pw) {
			...
			document.getElementById("pw_msg").innerHTML = "8자리~20자리 이내로 입력해주세요."
			$("#pw_check").attr('value', '0');
		}
		else if (num < 0 || eng < 0 || spe < 0) {
			document.getElementById("pw_msg").innerHTML = "영문자와 숫자, 특수기호가 최소 한 글자씩 필요합니다."
			$("#pw_check").attr('value', '0');
		} else {
			document.getElementById("pw_msg").innerHTML = "조건을 만족합니다."
	...
	$('#pw1').focusout(function() {
		if ($('#pw').val() == "" || $('#pw1').val() == "") {
			$('#pw_msg2').html('비밀번호를 다시 입력해주세요.');
			return false;
		}
		if ($('#pw').val() != $('#pw1').val()) {
			$('#pw_msg2').html('비밀번호가 일치하지 않습니다.');
			$("#pw_check2").attr('value', '0');
		} else {
			$('#pw_msg2').html('비밀번호가 일치합니다.');
			$("#pw_check2").attr('value', '1');
		}
	});
	...
	$("#submit_btn").click(function() {
		...
		if (document.getElementById("pw_check2").value != 1) {
			$("#pw1").focus();
			return false;
		}
